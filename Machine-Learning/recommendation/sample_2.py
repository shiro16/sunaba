...
def calc_similarity(dictionary, similarity_matrix, user_features):
    return[
        (movie_index, similarity) for movie_index, similarity in enumerate(similarity_matrix[dictionary.doc2bow(user_features)])
    ]
...
dictionary, similaritymatrix = create_similarity_matrix(MOVIES)
similarity = calc_similarity(dictionary, similaritymatrix, USER)
print(similarity)
